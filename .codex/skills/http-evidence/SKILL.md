...
- When a `response` is supplied, it also returns `response.security_findings`: CWE-tagged security-misconfiguration and session-cookie candidates derived purely from header presence/attribute flags -- missing `Strict-Transport-Security`/`X-Content-Type-Options`/`Content-Security-Policy`/clickjacking protection (CWE-319/693/1021), a `Set-Cookie` missing `Secure`/`HttpOnly`/`SameSite=Strict|Lax` (CWE-614/1004/1275), and a wildcard-or-reflected-origin `Access-Control-Allow-Origin` paired with `Access-Control-Allow-Credentials: true` (CWE-942). These are candidates like any other Detect-stage output -- register them via `mantis_findings` `finding_create` and still require reachability/attacker-simulation before `confirmed`.
